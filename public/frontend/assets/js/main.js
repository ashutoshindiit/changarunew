(function ($) {
    ...
    var  minGetData  = getUrlParameter('min_sidebar_filter');
    var  maxGetData = getUrlParameter('max_sidebar_filter');
    if(minGetData!='' && maxGetData!=''){
        $( "#slider-range" ).slider({
            range: true,
            min: 0,
            max: 1000,
            values: [ minGetData, maxGetData ]
            });
        $("#amount").val( "$" + minGetData + " - $" + maxGetData  );
        $("#min_sidebar_filter").val(ui.values[ minGetData ] );
        $("#max_sidebar_filter").val(ui.values[ maxGetData ] ); 
   ...
   /*---Newsletter Popup activation---*/
    ...
    $('.sidebarFilter a.filterBtn').on('click', function(){
        $('.newMini,.off_canvars_overlay').addClass('active')
    });
    
    $('.mini_cart_close,.off_canvars_overlay').on('click', function(){
        $('.newMini,.off_canvars_overlay').removeClass('active')
    });
